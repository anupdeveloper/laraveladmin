    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header alert d-flex justify-content-between align-items-center">
          <h3>{{ __('adminlte::adminlte.subject_list') }}</h3>
          @can('add_jobseeker')<a class="btn btn-sm btn-success" href="{{ route('add_subject') }}">{{ __('adminlte::adminlte.add_new') }}</a>@endcan
        </div>
        <div class="card-body">
          @if (session('status'))
          ...
          <table style="width:100%" id="jobseekers-list" class="table table-bordered table-hover">
            <thead>
              <tr>
                ...
                <th>{{ __('adminlte::adminlte.course_name') }}</th>
                <!--th>{{ __('adminlte::adminlte.price') }}</th-->
                <th>{{ __('adminlte::adminlte.duration') }}</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{ $list[$i]->get_course_detail[0]->name }}</td>
                <!--td>{{ $list[$i]->price ? $list[$i]->price : '' }}</td-->
...
@section('js')
  ...
  <script>
    ...
    $('.delete-button').click(function(e) {
      ...
      swal({
        title: "Are you sure?",
        text: "Are you sure you want to move this Subject to the Recycle Bin?",
        type: "warning",
