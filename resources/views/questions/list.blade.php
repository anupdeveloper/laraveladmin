    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header alert d-flex justify-content-between align-items-center">
          <h3>{{ __('adminlte::adminlte.list') }}</h3>
          @can('add_jobseeker')<a class="btn btn-sm btn-success" href="{{ route('add_question') }}">{{ __('adminlte::adminlte.add_new') }}</a>@endcan
        </div>
        <div class="card-body">
          @if (session('status'))
          ...
          <table style="width:100%" id="jobseekers-list" class="table table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr>
                  <th class="display-none"></th>
                <td>{{ $list[$i]->name }}</td>
                <td>{{ $list[$i]->get_school_name->name }}</td>
                <td>{{ $list[$i]->get_course_name->name }}</td>
                <td>
