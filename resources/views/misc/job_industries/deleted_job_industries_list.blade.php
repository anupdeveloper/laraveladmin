            ...
            <table style="width:100%" id="deletedJobIndustries" class="table table-bordered table-hover">
              <thead>
                <tr>
                  ...
                  @can('manage_recycle_bin_job_industries')
                  ...
                  @can('manage_recycle_bin_job_industries')
                    <td>
                      @can('restore_job_industries')
                        <a class="action-button restore-button" title="Restore" href="javascript:void(0)" data-id="{{ $deletedJobIndustries[$i]->id}}"><i class="text-success fa fa-undo"></i></a>
                      @endcan
                      @can('permanent_delete_job_industries')
                        <a class="action-button remove-button" title="Permanent Delete" href="javascript:void(0)" data-id="{{ $deletedJobIndustries[$i]->id}}"><i class="text-danger fa fa-trash"></i></a>
                      @endcan
                    </td>
                  @endcan
                </tr>
  ...
  <script>
    ...
    $('.remove-button').click(function(e) {
      ...
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "{{ route('permanent_delete_job_industry') }}",
            type: 'post',
            data: {
              id: id
            },
