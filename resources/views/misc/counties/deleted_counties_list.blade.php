          ...
          <div class="card-body">
            ...
            <table style="width:100%" id="deletedCounties" class="table table-bordered table-hover">
              ...
              <tr>
                  ...
                  @can('manage_recycle_bin_counties')
                    ...
                    @can('manage_recycle_bin_counties')
                      <td>
                        @can('restore_counties')
                          <a class="action-button restore-button" title="Restore" href="javascript:void(0)" data-id="{{ $deletedCounties[$i]->id}}"><i class="text-success fa fa-undo"></i></a>
                        @endcan
                        @can('permanent_delete_counties')
                          <a class="action-button remove-button" title="Permanent Delete" href="javascript:void(0)" data-id="{{ $deletedCounties[$i]->id}}"><i class="text-danger fa fa-trash"></i></a>
                        @endcan
                      </td>
                    @endcan
                  </tr>
  ...
  <script>
    ...
    $('.remove-button').click(function(e) {
      ...
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "{{ route('permanent_delete_county') }}",
            type: 'post',
            data: {
              id: id
            },
